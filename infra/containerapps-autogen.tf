resource "azurerm_container_app" "agenthost_app" {
  depends_on = [
    azurerm_role_assignment.agenthost_acr_pull
  ]

  name                         = "${project_name}-${var.environment}-agenthost-app"
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.agenthost_container_app_identity.id]
  }

  registry {
    server   = azurerm_container_app_environment.main.login_server
    identity = azurerm_user_assigned_identity.agenthost_container_app_identity.id
  }
...
resource "azurerm_container_app" "qdrant_app" {
  depends_on = [
    azurerm_role_assignment.qdrant_acr_pull
  ]

  name                         = "${project_name}-${var.environment}-qdrant-app"
  resource_group_name          = azurerm_resource_group.runtime_rg.name
  container_app_environment_id = azurerm_container_app_environment.main.id
  revision_mode                = "Single"
  workload_profile_name        = "consumption"
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.qdrant_container_app_identity.id]
  }

  registry {
    server   = azurerm_container_app_environment.main.login_server
    identity = azurerm_user_assigned_identity.qdrant_container_app_identity.id
  }
